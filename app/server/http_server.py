...
def create_app(engine):
    ...
    def signal():
        """
        统一信号接收入口（支持 BUY / SELL）。
        请求体: { "code": "sz000001", "action": "BUY", "type": "ACTIVE", "payload": {"price": 10.0} }
        """
        try:
            ...
            # 注意 HTTP 接口默认视为 BUY，如果是外部想要主动砸盘，应使用 action 参数
            action = data.get('action', 'BUY')
            queued = engine.submit(code, action=action, signal_type=signal_type, payload=payload)
            return jsonify({
                "success": True,
            ...
            logger.error(f"信号接收接口异常: {e}", exc_info=True)
            return jsonify({"success": False, "message": str(e)}), 500

    @app.route('/api/buy', methods=['POST'])
    def buy():
        """
        手动买入（不走策略，直接调 broker）。
        ...
        try:
            ...
            result = broker.buy(code, volume, price)
